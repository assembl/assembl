...
def check_and_create_database_user():
    ...
    with settings(warn_only=True):
        checkUser = run("yes|PGPASSWORD=%s psql --host=%s --username=%s -tAc \"SELECT 1 FROM pg_roles WHERE rolname='%s'\"" % (env.db_password, env.db_host, env.db_user, env.db_user), pty=False)
    if checkUser.failed:
...
def database_delete_postgres():
    ...
    with settings(warn_only=True), hide('stdout'):
        checkDatabase = run("PGPASSWORD=%s psql -n --host=%s --username=%s --dbname=%s -l" % (
