...
VERSIONED = 0  # default
VERSION_IF_CHANGED = 1  # store only old values if changed
VERSION_META = 2  # do not store
VERSION_NONE = 3  # do not store
...
def _do_not_version(col):
    return col.info.get(VERSIONING, VERSIONED) in (VERSION_META, VERSION_NONE)
...
def _history_mapper(local_mapper):
    ...
    if not super_mapper or local_mapper.local_table is not super_mapper.local_table:
        ...
        version_meta = {VERSIONING: VERSION_META}  # add column.info to identify
    ...
    # Mostly for DiscussionBoundBase
    methods = {'__history_table__': True}
    for base in bases:
        if getattr(base, '__abstract__', None):
            for method_name, method in base.__dict__.items():
                if getattr(method, '__isabstractmethod__', None):
                    methods[method_name] = getattr(getattr(cls, method_name), 'im_func')
    versioned_cls = type.__new__(type, "%sHistory" % cls.__name__, bases, methods)
...
def declare_history_mappers():
    for mp in _versioned_mappers:
        _history_mapper(mp)
    ...
    for om, hm in zip(obj_mapper.iterate_to_root(), history_mapper.iterate_to_root()):
        ...
        for hist_col in hm.local_table.c:
            ...
            if prop.key not in obj_state.dict:
                if obj_col.info.get(VERSIONING, VERSIONED) == VERSION_IF_CHANGED:
                    continue
