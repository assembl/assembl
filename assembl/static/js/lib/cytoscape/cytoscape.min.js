...
/**
 * This file is part of cytoscape.js github-snapshot-2013.12.12-19.17.46.
 * 
...
})(cytoscape);(function(a){a.fn.core({on:a.define.on(),one:a.define.on({unbindSelfOnTrigger:true}),once:a.define.on({unbindAllBindersOnTrigger:true}),off:a.define.off(),trigger:a.define.trigger(),});a.corefn.bind=a.corefn.on;a.corefn.unbind=a.corefn.off;a.define.event.aliasesOn(a.corefn)})(cytoscape);(function(a){a.fn.core({png:function(b){var d=this;var c=this._private.renderer;return c.png(b)}})})(cytoscape);(function(a){a.fn.core({layout:function(b){var c=this;if(this._private.layoutRunning){return this
...
var l=c.clientHeight;var f,g=e,k=l;var d=this.getPixelRatio();g*=d;k*=d;for(var h=0;h<a.CANVAS_LAYERS;h++){f=j.canvases[h];if(f.width!==g||f.height!==k){f.width=g;f.height=k;f.style.width=e+"px";f.style.height=l+"px"}}for(var h=0;h<a.BUFFER_COUNT;h++){f=j.bufferCanvases[h];if(f.width!==g||f.height!==k){f.width=g;f.height=k}}this.data.overlay.style.width=e+"px";this.data.overlay.style.height=l+"px"};a.prototype.redraw=function(k,h,p,f){var c=this;var d=this.getPixelRatio();if(this.averageRedrawTime===undefined){this.averageRedrawTime=0
}var g=1000/60;var o=1000;var l=this.averageRedrawTime;l=Math.max(g,l);l=Math.min(l,o);if(this.lastDrawTime===undefined){this.lastDrawTime=0}var m=+new Date;var e=m-this.lastDrawTime;var j=e>=l;if(!k){if(!j){clearTimeout(this.redrawTimeout);this.redrawTimeout=setTimeout(function(){c.redraw()},l);return}this.lastDrawTime=m}setTimeout(function(){var K=m;var F=100;var u=c.data.cy;var T=c.data;var N=c.getCachedNodes();var t=c.getCachedEdges();c.matchCanvasSize(T.container);var q=u.zoom();var E=p!==undefined?p:q;
var M=u.pan();var Q={x:M.x,y:M.y};if(f){Q=f}E*=d;Q.x*=d;Q.y*=d;var J=[];for(var O=0;O<N.length;O++){J.push(N[O])}for(var O=0;O<t.length;O++){J.push(t[O])}if(T.canvasNeedsRedraw[a.DRAG]||T.canvasNeedsRedraw[a.NODE]||h){if(c.hideEdgesOnViewport&&(c.pinching||c.hoverData.dragging||c.data.wheel||c.swipePanning)){}else{c.findEdgeControlPoints(t)}var J=c.getCachedZSortedEles();if(u.hasCompoundNodes()){c.updateAllCompounds(J)}}var C;var z;var s;if(T.canvasNeedsRedraw[a.NODE]||h){if(!C||!z){C=[];z=[];for(var G=0;
G<J.length;G++){s=J[G];if(s._private.rscratch.inDragLayer){C.push(s)}else{z.push(s)}}}var v=k||T.canvases[a.NODE].getContext("2d");v.setTransform(1,0,0,1,0,0);v.clearRect(0,0,v.canvas.width,v.canvas.height);if(!h){v.translate(Q.x,Q.y);v.scale(E,E)}if(f){v.translate(f.x,f.y)}if(p){v.scale(p,p)}for(var G=0;G<z.length;G++){s=z[G];if(s._private.group=="nodes"){c.drawNode(v,s)}else{if(s._private.group=="edges"){c.drawEdge(v,s)}}}for(var G=0;G<z.length;G++){s=z[G];if(s._private.group=="nodes"){c.drawNodeText(v,s)
}else{if(s._private.group=="edges"){c.drawEdgeText(v,s)}}if(s._private.group=="nodes"){c.drawNode(v,s,true)}else{if(s._private.group=="edges"){c.drawEdge(v,s,true)}}}if(!h){T.canvasNeedsRedraw[a.NODE]=false;T.canvasRedrawReason[a.NODE]=[]}}if(T.canvasNeedsRedraw[a.DRAG]||h){if(!C||!z){C=[];z=[];for(var G=0;G<J.length;G++){s=J[G];if(s._private.rscratch.inDragLayer){C.push(s)}else{z.push(s)}}}var v=k||T.canvases[a.DRAG].getContext("2d");if(!h){v.setTransform(1,0,0,1,0,0);v.clearRect(0,0,v.canvas.width,v.canvas.height);
v.translate(Q.x,Q.y);v.scale(E,E)}if(f){v.translate(f.x,f.y)}if(p){v.scale(p,p)}var s;for(var G=0;G<C.length;G++){s=C[G];if(s._private.group=="nodes"){c.drawNode(v,s)}else{if(s._private.group=="edges"){c.drawEdge(v,s)}}}for(var G=0;G<C.length;G++){s=C[G];if(s._private.group=="nodes"){c.drawNodeText(v,s)}else{if(s._private.group=="edges"){c.drawEdgeText(v,s)}}if(s._private.group=="nodes"){c.drawNode(v,s,true)}else{if(s._private.group=="edges"){c.drawEdge(v,s,true)}}}if(!h){T.canvasNeedsRedraw[a.DRAG]=false;
T.canvasRedrawReason[a.DRAG]=[]}}if(T.canvasNeedsRedraw[a.SELECT_BOX]){var v=k||T.canvases[a.SELECT_BOX].getContext("2d");if(!h){v.setTransform(1,0,0,1,0,0);v.clearRect(0,0,v.canvas.width,v.canvas.height);v.translate(Q.x,Q.y);v.scale(E,E)}if(f){v.translate(f.x,f.y)}if(p){v.scale(p,p)}var r=u.style()._private.coreStyle;if(T.select[4]==1){var q=T.cy.zoom();var S=r["selection-box-border-width"].value/q;v.lineWidth=S;v.fillStyle="rgba("+r["selection-box-color"].value[0]+","+r["selection-box-color"].value[1]+","+r["selection-box-color"].value[2]+","+r["selection-box-opacity"].value+")";
v.fillRect(T.select[0],T.select[1],T.select[2]-T.select[0],T.select[3]-T.select[1]);if(S>0){v.strokeStyle="rgba("+r["selection-box-border-color"].value[0]+","+r["selection-box-border-color"].value[1]+","+r["selection-box-border-color"].value[2]+","+r["selection-box-opacity"].value+")";v.strokeRect(T.select[0],T.select[1],T.select[2]-T.select[0],T.select[3]-T.select[1])}}if(T.bgActivePosistion){var q=T.cy.zoom();var D=T.bgActivePosistion;v.fillStyle="rgba("+r["active-bg-color"].value[0]+","+r["active-bg-color"].value[1]+","+r["active-bg-color"].value[2]+","+r["active-bg-opacity"].value+")";
v.beginPath();v.arc(D.x,D.y,r["active-bg-size"].pxValue/q,0,2*Math.PI);v.fill()}if(!h){T.canvasNeedsRedraw[a.SELECT_BOX]=false;T.canvasRedrawReason[a.SELECT_BOX]=[]}}if(c.options.showOverlay){var v=T.canvases[a.OVERLAY].getContext("2d");v.lineJoin="round";v.font="14px helvetica";v.strokeStyle="#fff";v.lineWidth="4";v.fillStyle="#666";v.textAlign="right";var I="cytoscape.js";var H=v.canvas.width;var P=v.canvas.height;var L=4;var R=v.measureText(I).width;var B=14;v.clearRect(0,0,H,P);v.strokeText(I,H-L,P-L);
v.fillText(I,H-L,P-L);T.overlayDrawn=true}var A=+new Date;if(c.averageRedrawTime===undefined){c.averageRedrawTime=A-K}c.averageRedrawTime=c.averageRedrawTime/2+(A-K)/2;if(!k&&!c.initrender){c.initrender=true;u.trigger("initrender")}},0)}})(cytoscape);(function(b){var a=b("renderer","canvas");a.prototype.drawPolygonPath=function(f,d,j,h,c,g){f.translate(d,j);f.scale(h/2,c/2);f.beginPath();f.moveTo(g[0],g[1]);for(var e=1;e<g.length/2;e++){f.lineTo(g[e*2],g[e*2+1])}f.closePath();f.scale(2/h,2/c);f.translate(-d,-j)
...
(function(b){var a=b("renderer","canvas");a.prototype.createBuffer=function(d,e){var c=document.createElement("canvas");c.width=d;c.height=e;return[c,c.getContext("2d")]};a.prototype.png=function(d){var h=this.data;var k=h.cy;var g=this.data.container.clientWidth;var c=this.data.container.clientHeight;var j=this.data.bufferCanvases[1];var f=h.bufferCanvases[1].getContext("2d");if(g>0&&c>0){f.clearRect(0,0,g,c);if(d.bg){f.fillStyle=d.bg;f.rect(0,0,g,c);f.fill()}f.globalCompositeOperation="source-over";
for(var e=a.CANVAS_LAYERS-1;e>=0;e--){if(e===a.OVERLAY){continue}f.drawImage(h.canvases[e],0,0)}}return j.toDataURL("image/png")};a.prototype.renderTo=function(c,d,e){this.redraw(c,true,d,e)}})(cytoscape);(function(b){var a=b("renderer","canvas");a.prototype.registerBinding=function(f,e,d,c){this.bindings.push({target:f,event:e,handler:d,useCapture:c});f.addEventListener(e,d,c)};a.prototype.load=function(){var p=this;var m=function(D,r,E){if(!r){var B=D.parents();for(var C=0;C<B.size();C++){if(B[C]._private.selected){return
}}}var F=D.descendants();function A(H){while(H.parent().nonempty()&&H.parent().id()!==D.id()){parent=H.parent()[0];var G=parent._private.style;if(G.width.value!=="auto"||G.height.value!=="auto"){return true}H=H.parent()}return false}for(var C=0;C<F.size();C++){if((r||!F[C]._private.selected)){F[C]._private.rscratch.inDragLayer=true;E.push(F[C]);for(var z=0;z<F[C]._private.edges.length;z++){F[C]._private.edges[z]._private.rscratch.inDragLayer=true}}}};var q=function(z,A){z._private.grabbed=true;z._private.rscratch.inDragLayer=true;
A.push(z);for(var r=0;r<z._private.edges.length;r++){z._private.edges[r]._private.rscratch.inDragLayer=true}};var v=function(D,B){var C=D;while(C.parent().nonempty()){C=C.parent()[0]}if(C==D&&B){return}var r=C.descendants().add(C);for(var A=0;A<r.size();A++){r[A]._private.rscratch.inDragLayer=B;for(var z=0;z<r[A]._private.edges.length;z++){r[A]._private.edges[z]._private.rscratch.inDragLayer=B}}};a.prototype.nodeIsDraggable=function(r){if(r._private.style.opacity.value!=0&&r._private.style.visibility.value=="visible"&&r._private.style.display.value=="element"&&!r._private.locked&&r._private.grabbable){return true
}return false};p.registerBinding(window,"resize",function(r){p.data.canvasNeedsRedraw[a.NODE]=true;p.data.canvasNeedsRedraw[a.OVERLAY]=true;p.matchCanvasSize(p.data.container);p.redraw()},true);p.registerBinding(p.data.container,"contextmenu",function(r){r.preventDefault()});p.registerBinding(p.data.container,"mousedown",function(B){B.preventDefault();p.hoverData.capture=true;p.hoverData.which=B.which;var r=p.data.cy;var E=p.projectIntoViewport(B.pageX,B.pageY);var F=p.data.select;var A=p.findNearestElement(E[0],E[1],true);
var H=p.hoverData.down;var C=p.dragData.possibleDragElements;var D=new b.Event(B,{type:"grab"});if(B.which==3){if(A){A.activate();A.trigger(new b.Event(B,{type:"cxttapstart"}));p.hoverData.down=A;p.hoverData.downTime=(new Date()).getTime();p.hoverData.cxtDragged=false}}else{if(B.which==1){if(A){A.activate()}if(A!=null&&p.nodeIsDraggable(A)){if(A._private.group=="nodes"&&A._private.selected==false){C=p.dragData.possibleDragElements=[];q(A,C);A.trigger(D);if(A._private.style.width.value=="auto"||A._private.style.height.value=="auto"){m(A,true,C)
}v(A,true)}if(A._private.group=="nodes"&&A._private.selected==true){C=p.dragData.possibleDragElements=[];var I=false;var G=r.$("node:selected");for(var z=0;z<G.length;z++){if(p.nodeIsDraggable(G[z])){q(G[z],C);if(!I){A.trigger(D);I=true}if(G[z]._private.style.width.value=="auto"||G[z]._private.style.height.value=="auto"){m(G[z],false,C)}v(G[z],true)}}}A.trigger(new b.Event(B,{type:"mousedown"})).trigger(new b.Event(B,{type:"tapstart"})).trigger(new b.Event(B,{type:"vmousedown"}))}else{if(A==null){r.trigger(new b.Event(B,{type:"mousedown"})).trigger(new b.Event(B,{type:"tapstart"})).trigger(new b.Event(B,{type:"vmousedown"}))
...
var z=[B[0]*C.zoom()+C.pan().x,B[1]*C.zoom()+C.pan().y];if(true||p.zoomData.freeToZoom){A.preventDefault();var r=A.wheelDeltaY/1000||A.wheelDelta/1000||A.detail/-32||-A.deltaY/500;if(C.panningEnabled()&&C.zoomingEnabled()){C.zoom({level:C.zoom()*Math.pow(10,r),position:{x:z[0],y:z[1]}})}p.data.wheel=true;clearTimeout(p.data.wheelTimeout);p.data.wheelTimeout=setTimeout(function(){p.data.wheel=false;p.data.canvasNeedsRedraw[a.NODE]=true;p.redraw()},100)}};p.registerBinding(p.data.container,"wheel",j,true);
p.registerBinding(p.data.container,"mousewheel",j,true);p.registerBinding(p.data.container,"DOMMouseScroll",j,true);p.registerBinding(p.data.container,"MozMousePixelScroll",function(r){if(p.zoomData.freeToZoom){r.preventDefault()}},false);p.registerBinding(p.data.container,"mousemove",function(r){if(p.zoomData.lastPointerX&&p.zoomData.lastPointerX!=r.pageX&&!p.zoomData.freeToZoom){p.zoomData.freeToZoom=true}p.zoomData.lastPointerX=r.pageX},false);p.registerBinding(p.data.container,"mouseout",function(r){p.zoomData.freeToZoom=false;
p.zoomData.lastPointerX=null},false);p.registerBinding(p.data.container,"mouseout",function(r){p.data.cy.trigger(new b.Event(r,{type:"mouseout"}))},false);p.registerBinding(p.data.container,"mouseover",function(r){p.data.cy.trigger(new b.Event(r,{type:"mouseover"}))},false);var d,k,s,w;var t;var g,f;var h,u;var l=p.data.container.clientWidth,o=p.data.container.clientHeight;var c;function e(z,B,r,A){return Math.sqrt((r-z)*(r-z)+(A-B)*(A-B))}p.registerBinding(p.data.container,"touchstart",function(T){clearTimeout(this.threeFingerSelectTimeout);
...
C="Node: "+lnode.id+". Refreshed position: ("+lnode.positionX+", "+lnode.positionY+").";r(C);return{x:lnode.positionX,y:lnode.positionY}});if(true!=b.ready){C="Triggering layoutready";r(C);b.ready=true;F.one("layoutready",B.ready);F.trigger("layoutready")}}function e(C,D,z,B){var A="\n\n###############################";A+="\nSTEP: "+B;A+="\n###############################\n";r(A);w(C,D,z);v(C,D,z);q(C,D,z);p(C,D,z);g(C,D,z)}function w(B,A,J){var I="calculateNodeForces";r(I);for(var E=0;E<B.graphSet.length;
